...
**Here is our replication (only a template, to be upated with recoded data):** 
![](Images/figure2_replication.tiff)
...
#calculate mean ed for figure 2
mean_ed_graph <- colorr_recode_subset %>%
  select(colorr_recode, pais, region, ed) %>%
  filter(!is.na(ed)) %>%
  group_by(colorr_recode, pais, region) %>%
  summarize(mean_ed = mean(ed))
View(mean_ed_graph)

#figure 2 
mean_ed_graph$region <- factor(mean_ed_graph$region, levels = c("Central America and Mexico", "Andean", "Southern Cone and Brazil", "Caribbean"))
levels(mean_ed_graph$region)
...
![](Images/Figure3_replication.tiff)
...
#To Do- reorder the position or countries based on difference in mean between dark and light skin
...
![](Images/Appendix_replication.tiff)
...
![](Images/Figure4_replication.tiff)
...
#weights? 
names(clean.2010)
clean.2010$weight1500

class(eight_pais$Female)

reg1 <- lm(scale(ed)~ scale(colorr) + scale(parent_occ) + scale(Female) + scale(q2) + scale(Urban) + scale(Mexico) + scale(Guatemala) + scale(Colombia) + scale(Ecuador) + scale(Bolivia) + scale(Peru) + scale(Dominican_Republic), data=eight_pais)
