...
pais_ed <- colorr_recode_subset %>%
  ...
  group_by(pais, tone) %>%
  select(pais, tone, ed, weight1500, upm, estratopri) 
 
pais_ed$newid <- paste(pais_ed$upm, pais_ed$estratopri)
mydesign <- svydesign(ids=~newid, strata=~ estratopri, data=pais_ed, weights=~weight1500)
mean_edu <- svyby(~ed, ~pais+tone, mydesign, svymean, na.rm=T)
conf <- confint(mean_edu) 
pais_edu <- cbind(mean_edu, conf)
pais_edu <- pais_edu %>%
  rename(ci.lower = `2.5 %`, ci.upper = `97.5 %`)


#fig 3 prep, order countries 
pais_edu$pais <- factor(pais_edu$pais,
                levels = c("Bolivia", 
                           "Uruguay", 
                           "Peru", 
                           "Guatemala", 
...
#Figure 3
fig3 <- ggplot(pais_edu, aes( x = factor(tone), y = ed, fill = tone)) +
  ylim(0, 14) +
  ylab("Years of Schooling") + xlab("") +
  geom_bar(stat ="identity") + 
  geom_errorbar(aes(ymin = ci.lower, ymax = ci.upper)) + 
  facet_wrap(~ pais, ncol = 2) + coord_flip() +
  geom_text(aes(label = round(ed, 1)), size = 3.5, hjust = -.7) +
...
for (name in country.names) {
  country.graph[[name]] <- lm(scale(ed) ~ scale(colorr) + 
                               ...
                               scale(Urban) + 
                               scale(q2) + 
                               scale(Female), data= filter(eight_pais, pais == name)) 
...
country_grid <- arrangeGrob(country.fig.note.brazil, 
