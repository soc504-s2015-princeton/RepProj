...
#practice code
#grab mean ed
mean_ed_graph <- clean.2010 %>%
  mutate(colorr=as.numeric(colorr)) %>%
  group_by(colorr, pais, region) %>%
  summarise(mean_ed = mean(ed))

#figure 2 template, but with wrong colorr var
#fix legend
...
p <- ggplot(data= mean_ed_graph, aes(x=colorr, y=mean_ed, color=pais))
p+ layer (geom="line") +
